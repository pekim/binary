...
const BinaryBufferReader = function(endian) {
  ...
  this.ensureBytesAvailable = function(numberOfBytes) {
    ...
    var bytesNeededInBuffer = this.offset + numberOfBytes;
    while ((this.buffers[0].length < bytesNeededInBuffer) && (this.buffers.length > 1)) {
      this.collapseFrontTwoBuffers();
      
      this.offset = 0;
      bytesNeededInBuffer = this.offset + numberOfBytes;
    }
  ...
  this.collapseFrontTwoBuffers = function() {
    var newBuffer = new Buffer((this.buffers[0].length - this.offset) + this.buffers[1].length);
    this.buffers[0].copy(newBuffer, 0, this.offset);
    this.buffers[1].copy(newBuffer, this.buffers[0].length - this.offset, 0);
